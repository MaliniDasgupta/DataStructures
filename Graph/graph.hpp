...
private:
    ...
    //methods
    std::vector<int> helperAdjMatrix(std::string line, int lineNo) const;
    std::vector<int> helperAdjList(std::string line) const;
    std::vector<std::vector<int>> helperUnweightedListEdge(const std::vector<std::pair<unsigned, unsigned>>& edges) const;
    std::vector<std::vector<int>> helperWeightedListEdge(const std::vector<std::tuple<unsigned, unsigned, int>>& edges) const;
    //std::vector<int> PrimsAlgorithm(int start) const;
};
